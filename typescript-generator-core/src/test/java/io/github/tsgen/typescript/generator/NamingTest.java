...
import io.github.tsgen.typescript.generator.yield.KeywordInPackage;
...
public class NamingTest {
    ...
    public void testConflictResolved() {
        ...
        settings.customTypeNaming.put("io.github.tsgen.typescript.generator.NamingTest$A$ConflictingClass", "A$ConflictingClass");
        settings.customTypeNaming.put("io.github.tsgen.typescript.generator.NamingTest$B$ConflictingClass", "B$ConflictingClass");
    ...
    public void testConflictPrevented() {
        ...
        final String output = new TypeScriptGenerator(settings).generateTypeScript(Input.from(A.ConflictingClass.class, B.ConflictingClass.class));
        Assertions.assertTrue(output.contains("namespace io.github.tsgen.typescript.generator.NamingTest.A {"));
        Assertions.assertTrue(output.contains("namespace io.github.tsgen.typescript.generator.NamingTest.B {"));
    }
    ...
    public void testTypeNamingFunction() {
        final Settings settings = TestUtils.settings();
        settings.customTypeNamingFunction = "function(name, simpleName) { if (name.indexOf('io.') === 0) return 'Test' + simpleName; }";
    ...
    public void testCombinations() {
        final Settings settings = TestUtils.settings();
        settings.customTypeNamingFunction = "function(name, simpleName) { if (name.indexOf('io.') === 0) return 'Func' + simpleName; }";
    ...
    public void testTypeScriptKeywords() {
        ...
        Assertions.assertEquals("io.github.tsgen.typescript.generator._yield.KeywordInPackage", name);
    }

    @Test
    public void testNamespaced() {
        final Settings settings = TestUtils.settings();
        settings.customTypeNaming = Collections.singletonMap("io.github.tsgen.typescript.generator.NamingTest$C", "NS.C");
